...
## 3. Helm chart
Helm Chart for deploy the image from Docker Hub. the chart have flexibility to allow Developer to adjust Values without having to rebuild the chart frequently.
...
Use **_kubectl apply -f_** with generated file from **_helm template_** to deploying to Kubernetes.
```sh
...
Setup EKS cluster with related resources to run EKS like VPC, Subnet, etc. by following EKS best practice using Terraform tools.
...
2. Generates a speculative execution plan, showing what actions Terraform would take to apply the current configuration.
   ...
   terraform plan
   ```
5. Provision or updates all the resources we have declared in the directory and module file.
...
### Create a Terraform module for IAM and installed with EKS Node group.

Example for iam role, iam role policy, and iam role policy attachment for accessing to S3 and SQS in Terrform format.
...
- SQS with permission; send, receive, delete message (SQS _ARN:arn:aws:sqs:ap-southeast-1:123456789123:lms-import-data_)
  ...
  resource "aws_iam_role_policy_attachment" "sqs" {
   ...
   ```
## 6. ArgoCD
Use ArgoCD to deploy Go application.

### How to deploy ArgoCD application
...
5. Another option to create application is by writing a YAML file and use kubectl apply to deploy ArgoCD application.
   ...
   This YAML file generate by Helm template cli, you can run this command to generate.
...
Github action workflow for deploy application with GitOps.

![action_image](png/actions.png)

1. Build image from Dockerfile and go code.
2. Run image vulnerability scanner for bugs or vulnerability detection.
3. Push image to Docker Hub
4. Generate K8S manifest from Helm chart and adjustable Values file (values.yaml)
5. Push generated manifest file to GitOps repository for automated sync modified resources with ArgoCD.
